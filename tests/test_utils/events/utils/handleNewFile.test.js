...
test("handleNewFile",  async () => {
    ...
    *
    * */
    handleNewFile(repoPath, DEFAULT_BRANCH, newFilePath, shadowRepoPath, originalsRepoPath, diffsRepo);
...
test("handleNewFile with syncignored file",  async () => {
    handleNewFile(repoPath, DEFAULT_BRANCH, ignorableFilePath, shadowRepoPath, originalsRepoPath, diffsRepo);
    // Verify file has been created in the .shadow repo and .originals repos
...
test("handleNewFile with shadow file there",  async () => {
...
test("handleNewFile with originals file there",  async () => {
...
test("handleNewFile with new directory",  async () => {
    fs.mkdirSync(newDirectoryPath, { recursive: true });
    handleNewFile(repoPath, DEFAULT_BRANCH, newDirectoryPath, shadowRepoPath, originalsRepoPath, diffsRepo);
    // Verify correct diff file has been generated
    let diffFiles = fs.readdirSync(diffsRepo);
    expect(diffFiles).toHaveLength(0);
});
