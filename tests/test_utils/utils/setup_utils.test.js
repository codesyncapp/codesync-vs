...
describe("setupCodeSync",  () => {
    ...
    test('with empty user.yml',  async () => {
        ...
        expect(vscode.window.showInformationMessage.mock.calls[1][0]).toBe(NOTIFICATION.WELCOME_MSG);
        expect(vscode.window.showInformationMessage.mock.calls[1][1]).toBe(NOTIFICATION.JOIN);
        expect(vscode.window.showInformationMessage.mock.calls[1][2]).toBe(NOTIFICATION.IGNORE);
    ...
    test('with user and repo not synced',  async () => {
        ...
        expect(vscode.window.showInformationMessage.mock.calls[2][0]).toBe(NOTIFICATION.CONNECT_REPO);
        expect(vscode.window.showInformationMessage.mock.calls[2][1]).toBe(NOTIFICATION.CONNECT);
        expect(vscode.window.showInformationMessage.mock.calls[2][2]).toBe(NOTIFICATION.IGNORE);
        fs.rmSync(userFilePath);
    });
    ...
    test('with synced repo',  async () => {
        ...
        expect(vscode.window.showInformationMessage).toHaveBeenCalledTimes(4);
        expect(vscode.window.showInformationMessage.mock.calls[3][0]).toBe(NOTIFICATION.REPO_IN_SYNC);
        expect(vscode.window.showInformationMessage.mock.calls[3][1]).toBe(NOTIFICATION.TRACK_IT);
        expect(vscode.window.showInformationMessage.mock.calls[3][2]).toBe(NOTIFICATION.UNSYNC_REPO);
        fs.rmSync(userFilePath);
    });
